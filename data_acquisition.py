...
def generateShadow(state, depth, number_of_snapshots, output_file_name, clifford=True, verbose=False):
    ...
    for k in range(number_of_shadows):
        ...
        measurement_result=sampleFromMPS(rotated_state) #the measurement result is a list of bits, 0 or 1 for each qubit

        classical_snapshot_tensors=[] #MPS tensors for the classical snapshot U^dagger|b>

        for i in range(size):

            local_tensors=empty((bond_dimension, bond_dimension, 4), dtype=complex)
        ...
        if(verbose):
            print('\r'+"Generated "+str(k+1)+ " out of "+ str(number_of_snapshots)+" snapshots.", end='' )

    if(verbose):
        print('\r'+"Generated "+str(number_of_snapshots)+ " out of "+ str(number_of_snapshots)+" snapshots.", end='' )
...
def randomCircuitMPO(size, depth, clifford=False):
    ...
    if(depth==0):
        ...
        for i in range(size):
            MPO_tensor=empty((bond_dimension, bond_dimension,4,4), dtype=complex)

            if(clifford):
                unitary_list=[random_clifford(2).to_matrix() for k in range(depth)]
            else:
                unitary_list=[array(unitary_group.rvs(4)) for k in range(depth)]

            for j in range(bond_dimension):
                for k in range(bond_dimension):
                    j_bin=int2list(j, depth-1)
                    k_bin=int2list(k, depth-1)
                    T=unitary_list[0]@transpose(kron(identity(2)[j_bin[0]], identity(2)))
                    for r in range(1,depth-1):
                        T=T@ kron(identity(2), identity(2)[k_bin[r-1]])@unitary_list[r]@transpose(kron(identity(2)[j_bin[r]], identity(2)))
                    MPO_tensor[j,k]=T@ kron(identity(2), identity(2)[k_bin[depth-2]])@unitary_list[depth-1]
...
def sampleFromMPS(M):
    ...
    for site in range(M.getSize()):
        ...
        for k in range(site):
            transfer_matrix=transfer_matrix@kron(M.bondMatrix(k,bits[k]), conj(M.bondMatrix(k,bits[k])))
        for k in range(M.getSize()-1, site, -1):
            transfer_matrix=sum(kron(M.bondMatrix(k,b), conj(M.bondMatrix(k,b)))  for b in range(M.getPhysicalDimension()))@transfer_matrix
        
        probabilities=[]
        for bit in range(M.getPhysicalDimension()):
            probabilities.append(abs(trace(transfer_matrix@kron(M.bondMatrix(site,bit), conj(M.bondMatrix(site,bit))))/normalization))
...
def sampleFromDistribution(probabilities):
    ...
    cumulative_distribution=[0]+[sum(probabilities[:k]) for k in range(1,size+1)]
    r=random()
    for k in range(size):
